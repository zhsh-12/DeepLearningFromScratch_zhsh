...
class DeepConvNet:
    ...
    def __init__(self, input_dim=(1, 28, 28), 
                 conv_param_1 = {'filter_num': 32, 'filter_size': 5, 'pad': 1, 'stride': 1},
                 conv_param_2 = {'filter_num': 64, 'filter_size': 5, 'pad': 1, 'stride': 1},
                 hidden_size = 50, output_size = 10): 
       #初始化权重
       pre_node_nums = np.array([1*5*5, 32*5*5, 64*5*5, hidden_size])
       ...
       self.params['W3'] = weight_init_scales[2] * np.random.randn(64*5*5, hidden_size)
    ...
    def predict(self, x, train_flg = False): 
        ...
        for layer in self.layers:
            if isinstance(layer, Dropout_2):
                x = layer.forward(x, train_flg) #Dropout层根据模式【训练/测试】决定是否执行随机丢弃神经元的操作
            else:
                x = layer.forward(x)
        return x 
